...
function select(r: { input: string; output: string }) {
  emit('select', r)
}

function clearAll() {
  historyStore.clear(props.tool)
}
</script>
...
<template>
  <div class="border rounded p-3" :style="{ backgroundColor: 'var(--bg-secondary)', borderColor: 'var(--border-color)' }">
    <div class="flex items-center justify-between mb-2">
      <h3 class="text-sm font-semibold">历史记录 ({{ records.length }})</h3>
      <button v-if="records.length" @click="clearAll" class="text-xs px-2 py-1 rounded" :style="{ backgroundColor: 'var(--btn-bg)', color: 'var(--text-primary)' }">清空</button>
    </div>
    <div class="space-y-2 max-h-[500px] overflow-y-auto">
      ...
      <div v-if="!records.length" class="text-xs text-center py-4" :style="{ color: 'var(--text-secondary)' }">
        暂无历史记录
      </div>
    </div>
  </div>
