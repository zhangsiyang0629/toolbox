...
const { t } = useLocale()
usePageMeta(t('tools.sql2gozero.title'), t('seo.sql2gozero'))
...
<template>
  <div class="max-w-7xl mx-auto px-4 py-4">
    ...
    <div class="flex items-center justify-between mb-2">
      <h1 class="text-xl font-bold">{{ t('tools.sql2gozero.title') }}</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4" :class="showHistory ? 'lg:grid-cols-3' : ''">
      <div class="flex flex-col">
        <label class="text-sm font-medium mb-1" :style="{ color: 'var(--text-secondary)' }">{{ t('tools.sql2gozero.input') }}</label>
        <div class="flex-1 min-h-[400px]">
          <CodeEditor v-model="input" lang="sql" />
        </div>
      </div>
      <div class="flex flex-col">
        <label class="text-sm font-medium mb-1" :style="{ color: 'var(--text-secondary)' }">{{ t('tools.sql2gozero.output') }}</label>
        <div class="flex-1 min-h-[400px]">
